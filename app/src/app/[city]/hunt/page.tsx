...
export default async function HuntPage({ params }: { params: Promise<{ city: string }> }) {
  ...
  const businessesById = new Map(((data.businesses as Array<{ id: string; address?: string; lon?: number; lat?: number }> | undefined) ?? []).map((business) => [business.id, business]));
  const stopsById = new Map((data.huntStops as HuntStopLite[] | undefined ?? []).map((stop) => {
    const business = businessesById.get(stop.business_id);
    return [stop.id, { ...stop, address: business?.address, lon: business?.lon, lat: business?.lat }];
  }));
