...
const products = [
  ...
  ["race", "Loop Race", "$99", "Up to 4 teams, one host payment, shared join code"],
  ["private", "Private Event", "$199", "Up to 6 teams, own start time, custom finish, printed materials"],
...
export default function EventsPage() {
  ...
  const [status, setStatus] = useState("No calendar integration in v1: request a date, pay/test-checkout, then confirm by email.");
  ...
  const submit = async () => {
    ...
    setStatus(`${json.data.label}: ${json.data.checkout_mode}. ${json.data.stripe_note} Confirmation path: ${json.data.checkout_url}`);
  };
    ...
    <main className={styles.landing}>
      ...
      <div className={styles.landWrap}>
        ...
        <section className={styles.landBlk}><div className={styles.landDuo}>
          ...
          </div><div className={styles.landHeroCta}><button className={`${styles.btn} ${styles.btnPrimary}`} onClick={submit}>Create test checkout</button><Link className={`${styles.btn} ${styles.btnGhost}`} href="/calgary/hunt">Try a hunt first</Link></div><p className={styles.landCardP}>{status}</p></div>
